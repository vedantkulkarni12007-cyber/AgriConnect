...
def list_offers(request: Request, db: Session = Depends(get_db), lot_id: str | None = None, status: str | None = None, page: int = Query(1, ge=1), limit: int = Query(20, ge=1, le=100), user: User = Depends(get_current_user)):
    ...
    # farmers see offers for their lots, buyers see their own - filter by ownership if not admin
...
def counter_offer(offer_id: str, data: CounterRequest, request: Request, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    new_offer=Offer(id=uuid.uuid4(), lot_id=orig.lot_id, buyer_id=orig.buyer_id, owner_id=orig.owner_id, quantity=data.quantity or float(orig.quantity), price_per_unit=data.price_per_unit or float(orig.price_per_unit), total_value=(data.quantity or float(orig.quantity))*(data.price_per_unit or float(orig.price_per_unit)), message=data.message or orig.message, status="PENDING", parent_offer_id=orig.id, expires_at=datetime.now(timezone.utc)+timedelta(days=3))
    db.add(new_offer)
    rid=getattr(request.state,"request_id",str(uuid.uuid4()))
